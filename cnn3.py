...
for dense_layer in dense_layers:
    for layer_size in layer_sizes:
        for conv_layer in conv_layers:
            model_name = "{}-conv-{}-filters-{}-dense".format(conv_layer, layer_size, dense_layer)
            ...
            for l in range(conv_layer - 1):
                model.add(Conv1D(layer_size, kernel_size=3, strides=1, padding='same', kernel_regularizer=l2(1e-4),
                                 input_shape=input_shape))
                model.add(BatchNormalization())
                model.add(Activation('relu'))
            ...
            plot_model(model=model,
                       to_file="images/{}-conv-{}-filters-{}-dense.png".format(conv_layer, layer_size, dense_layer),
                       show_shapes=True)
